...
import com.mindtree.adminService.exceptions.InvalidSeatNumber;
...
import com.mindtree.adminService.exceptions.SeatAlreadyBooked;
...
public class AdminServiceImpl implements AdminService {
	...
	public ShowScreenDetailsResponseDto addShowScreenDetailsDB(ShowScreenDetailsRequestDto showScreenDetailsRequestDto)
		...
		showScreenDetails.setBookedSeats(new ArrayList<String>(showScreenDetails.getTotalSeatsPresent()));
...
//		showScreenDetails.getBookedSeats().addAll(screenSeatUpdatedRequestDto.getBookedSeats());

		for (String seat : screenSeatUpdatedRequestDto.getBookedSeats()) {
			int seatNum = Integer.parseInt(seat);
			if (seatNum < 0 && seatNum > showScreenDetails.getTotalSeatsPresent()) {
				throw new InvalidSeatNumber(
						"Invalid Seat Number Please Enter in range of 1 to screen capacity of seat");
			}

			if (showScreenDetails.getBookedSeats().stream().anyMatch(s->s.equals(seat))) {
				
				System.out.println(showScreenDetails.getBookedSeats().size());
				
				throw new SeatAlreadyBooked("SeatNum " + seatNum + " is Already occupied");
			}

			showScreenDetails.getBookedSeats().add(seat);

		}

		showScreenDetails.setTotalSeatsPresent(
				showScreenDetails.getTotalSeatsPresent() - screenSeatUpdatedRequestDto.getBookedSeats().size());
	...
	@Override
	public ShowScreenSeatUpdatedRequestDto getBookedSeatInfoShowScreenFromDB(int showScreenId) throws ServiceException {
		ShowScreenDetails screenDetails = showScreenDetailsRepository.findById(showScreenId)
				.orElseThrow(() -> new ShowScreenDetailsIdInvalid("Invalid Show Screen Id"));

		ShowScreenSeatUpdatedRequestDto screenSeatUpdatedRequestDto = new ShowScreenSeatUpdatedRequestDto();
		screenSeatUpdatedRequestDto.setShowScreenDetailId(showScreenId);
		screenSeatUpdatedRequestDto.setBookedSeats(screenDetails.getBookedSeats());
		screenSeatUpdatedRequestDto.setScreenCapacity(screenDetails.getTotalSeatsPresent());
		return screenSeatUpdatedRequestDto;
	}
}
